...
#'
#'
FtDpv <- function(final.results, l2fc.co, pval.co) {
  # Label features with by DEM per group using pval
  final.labels <- final.results
  final.labels$A.DEM <- ifelse(abs(final.labels$d.facA) >= l2fc.co & final.labels$pval.exp.facA <= pval.co,"Yes", "No")
  final.labels$B.DEM <- ifelse(abs(final.labels$d.facB) >= l2fc.co & final.labels$pval.exp.facB <= pval.co,"Yes", "No")
  final.labels$AB.DEM <- ifelse(abs(final.labels$d.facAB) >= l2fc.co & final.labels$pval.exp.facAB <= pval.co,"Yes", "No")
