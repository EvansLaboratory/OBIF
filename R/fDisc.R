...
fDisc <- function(final.results, l2fc.co, pval.co, pval.sel) {
  ...
  DEMs.pval <- data.frame(
    A.DEM = ifelse (abs(final.results$d.facA) >= l2fc.co & final.results$pv.facA <= pval.co,"Yes", "No"),
    B.DEM = ifelse (abs(final.results$d.facB) >= l2fc.co & final.results$pv.facB <= pval.co,"Yes", "No"),
    AB.DEM = ifelse (abs(final.results$d.facAB) >= l2fc.co & final.results$pv.facAB <= pval.co, "Yes", "No"))

  # Label features with by DEM per group using BH
  DEMs.BH <- data.frame(
    A.DEM = ifelse (abs(final.results$d.facA) >= l2fc.co & final.results$adjpvBH.facA <= pval.co,"Yes", "No"),
    B.DEM = ifelse (abs(final.results$d.facB) >= l2fc.co & final.results$adjpvBH.facB <= pval.co,"Yes", "No"),
    AB.DEM = ifelse (abs(final.results$d.facAB) >= l2fc.co & final.results$adjpvBH.facAB <= pval.co, "Yes", "No"))

  # Label features with by DEM per group using Bonf
  DEMs.Bonf <- data.frame(
    A.DEM = ifelse (abs(final.results$d.facA) >= l2fc.co & final.results$adjpvBonf.facA <= pval.co,"Yes", "No"),
    B.DEM = ifelse (abs(final.results$d.facB) >= l2fc.co & final.results$adjpvBonf.facB <= pval.co,"Yes", "No"),
    AB.DEM = ifelse (abs(final.results$d.facAB) >= l2fc.co & final.results$adjpvBonf.facAB <= pval.co, "Yes", "No"))
  ...
  final.labels$AB.SME.A <- ifelse (final.labels$AB.DEM == "Yes" & final.labels$pv.SME.facA <= 0.05,
                                   "Yes", "No")
  final.labels$AB.SME.B <- ifelse (final.labels$AB.DEM == "Yes" & final.labels$pv.SME.facB <= 0.05,
                                   "Yes", "No")
  final.labels$AB.iDEM <- ifelse (final.labels$AB.DEM == "Yes" & final.labels$pv.Inter.fAxfB <= 0.05,
                                  "Yes", "No")
